...
const BranchAddress: React.FC = () => {
  ...
  const confirmDeleteBranch = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${baseUrl}/api/branches/${branchToDelete}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
                  ...
                  <table className="min-w-full bg-white border border-gray-200">
                    <thead>
                      <tr className="bg-gray-100">
                        <th className="py-2 px-2 sm:px-4 text-left font-semibold text-gray-700 border-b text-sm sm:text-base">
                          Branch Name
                        </th>
                        <th className="py-2 px-2 sm:px-4 text-left font-semibold text-gray-700 border-b text-sm sm:text-base">
                          Address
                    ...
                    <tbody>
                      {branches.map((branch) => (
                        <tr
                          key={branch.id}
                          className={`border-b ${
                            !branch.status ? "bg-gray-50 text-gray-500" : ""
                          }`}
                        >
                          <td className="py-2 px-2 sm:px-4 bg-white text-black text-sm sm:text-base">
                            {branch.name}
                          </td>
                          <td className="py-2 px-2 sm:px-4 bg-white text-black text-sm sm:text-base">
                            {branch.address}
                          ...
                          <td className="py-2 px-2 sm:px-4">
                            <button
                              className="px-2 sm:px-3 py-1 bg-red-600 text-black rounded hover:bg-red-900 transition-colors text-sm sm:text-base"
              ...
              <div className="mt-4 sm:mt-6">
                {!showCreateForm ? (
                  ...
                  <div className="bg-gray-800 p-3 sm:p-4 rounded-lg">
                    <h3 className="text-md sm:text-lg font-semibold text-white mb-3">
                      Add New Branch
                    </h3>
                    <input
                      type="text"
